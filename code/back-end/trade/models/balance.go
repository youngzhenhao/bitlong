...
type Balance struct {
	gorm.Model
	AccountId   int64   `gorm:"column:account_id" json:"accountId"` // 正确地将unique和column选项放在同一个gorm标签内
	BillType    int16   `gorm:"column:bill_type;type:smallint" json:"billType"`
	Away        uint    `gorm:"column:away;type:smallint" json:"away"`
	Amount      float64 `gorm:"column:amount;type:decimal(10,2)" json:"amount"`
	Unit        int     `gorm:"column:Unit;type:smallint" json:"unit"`
