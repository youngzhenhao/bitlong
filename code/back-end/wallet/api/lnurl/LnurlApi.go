...
func LnurlGetPortAvailable() string {
	return strconv.Itoa(frpc.RequestServerGetPortAvailable(base.QueryConfigByKey("LnurlServerHost")))
}
...
func LnurlFrpcRun(id, remotePort string) {
	if !frpc.RequestPostServerIsPortListening(remotePort) {
		frpc.FrpcConfig(id, remotePort)
		frpc.FrpcRun()
	} else {
...
func LnurlPayToLnu(lnu, amount string) string {
	...
	return lnd.SendPaymentSync(invoice)
}
