...
```
将2中创建的元数据对象作为参数传入assetMetaData，创建资产。
...
```
Meta meta = new Meta();
meta.FetchAssetMeta(false, assetID)
print(meta.description)//即可输出资产描述
```
isHash: 是否为元数据hash值
data: 元数据hash值或资产ID
...
7.解析资产元数据:保持资产元数据中的图像
```go
func (m *Meta) SaveImage(dir string, name string) bool
```
image_data为base64编码的图像数据，需要解码并保存到指定目录下
dir:保存图像的目录
name:保存图像文件名
