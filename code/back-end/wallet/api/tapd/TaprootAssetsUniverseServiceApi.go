...
func GetAssetInfo(id string) string {
	...
	// 获取时间戳
	block, err := lnd.GetBlock(proof.DecodedProof.Asset.ChainAnchor.AnchorBlockHash)
	if err != nil {
		return api.MakeJsonResult(false, err.Error(), nil)
	}
	...
	return api.MakeJsonResult(true, "", assetInfo)
}
...
func QueryAssetStats(assetId string) string {
	...
	if err != nil {
		...
		return api.MakeJsonResult(false, err.Error(), "")
	}
	return api.MakeJsonResult(true, "", response)
}
...
func SyncUniverse(universeHost string, asset_id string) string {
	...
	if response.SyncedUniverses == nil {
		return api.MakeJsonResult(false, "NOT_NEW_DATA", "")
	}
	return api.MakeJsonResult(true, "", response)
