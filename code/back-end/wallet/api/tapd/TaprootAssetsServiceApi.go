...
func FetchAssetMeta(isHash bool, data string) string {
	...
	return api.MakeJsonResult(true, "", hex.EncodeToString(response.Data))
}
...
func ListSimpleAssets(withWitness, includeSpent, includeLeased bool) string {
	...
	return api.MakeJsonResult(true, "", simpleAssets)
}
...
func FindAssetByAssetName(assetName string) string {
	...
	if response.Success == false {
		return api.MakeJsonResult(false, response.Error, nil)
	}
	...
	if len(assets) == 0 {
		return api.MakeJsonResult(false, "NOT_FOUND", nil)
	}
	return api.MakeJsonResult(true, "", assets)
}

func ListBalances() string {
	response, err := listBalances(false, nil, nil)
	if err != nil {
		api.MakeJsonResult(false, err.Error(), nil)
	}
	return api.MakeJsonResult(true, "", response)
}
