...
func GetNewAddress_P2TR() string {
	...
	if err != nil {
		...
		return api.MakeJsonResult(false, "AddressType_TAPROOT_PUBKEY error", "")
	}
	return api.MakeJsonResult(true, "", Addr{
...
func GetNewAddress_P2WKH() string {
	...
	if err != nil {
		fmt.Printf("%s lnrpc NewAddress err: %v\n", api.GetTimeNow(), err)
		return api.MakeJsonResult(false, "AddressType_WITNESS_PUBKEY_HASH error", "")
	}
	return api.MakeJsonResult(true, "", Addr{
		Address: response.Address,
		Balance: 0,
...
func GetNewAddress_NP2WKH() string {
	...
	if err != nil {
		fmt.Printf("%s lnrpc NewAddress err: %v\n", api.GetTimeNow(), err)
		return api.MakeJsonResult(false, "AddressType_NESTED_PUBKEY_HASH error", "")
	}
	return api.MakeJsonResult(true, "", Addr{
		Address: response.Address,
		Balance: 0,
...
func StoreAddr(address string, balance int, _type string) string {
	...
	if err != nil {
		return api.MakeJsonResult(false, "Store address fail", "")
	}
	return api.MakeJsonResult(true, "", address)
}
...
func RemoveAddr(address string) string {
	...
	if err != nil {
		return api.MakeJsonResult(false, "No such address available for deletion. Read Addr fail.", "")
	}
	err = s.DeleteAddr("addresses", address)
	if err != nil {
		return api.MakeJsonResult(false, "Delete Addr fail. "+err.Error(), "")
	}
	return api.MakeJsonResult(true, "", address)
}
...
func QueryAddr(address string) string {
	...
	if err != nil {
		return api.MakeJsonResult(false, "No such address, read Addr fail.", "")
	}
	return api.MakeJsonResult(true, "", addr)
}
...
func QueryAllAddr() string {
	...
	if err != nil || len(addresses) == 0 {
		return api.MakeJsonResult(false, "Addresses is NULL or read fail.", "")
	}
	return api.MakeJsonResult(true, "", addresses)
}
...
func GetNonZeroBalanceAddresses() string {
	...
	if err != nil {
		return api.MakeJsonResult(false, "Query addresses fail. "+err.Error(), "")
	}
	var addrs []Addr
	listAddrs := listAddrResp.GetAccountWithAddresses()
	if len(listAddrs) == 0 {
		return api.MakeJsonResult(false, "Queried non-zero balance addresses NULL.", "")
	}
	...
	return api.MakeJsonResult(true, "", addrs)
}
	...
	for _, accWithAddr := range listAddrs {
		...
		for _, _addr := range _addresses {
			if _addr.Balance != 0 {
				...
				var result api.JsonResult
				_re := StoreAddr(_ad, _ba, _ty)
				err := json.Unmarshal([]byte(_re), &result)
				if err != nil {
					return api.MakeJsonResult(false, "Store address Unmarshal fail. "+err.Error(), "")
				}
				if !result.Success {
					return api.MakeJsonResult(false, "Store address result false", "")
				}
